...
class Virtualizor_Admin_API
{
    ...
    public function iprange($page = 1, $reslen = 50, $post = [])
    {
        if (empty($post)) {
    ...
    public function orphaneddisks($post = [])
    {
    ...
    public function syncostemplate($post)
    {
    ...
    public function create_single_vps_backup($vpsid)
    {
    ...
    public function import_solusvm($post)
    {
        ...
        foreach ($res as $k => $v) {
            if (in_array($k, ['title', 'error', 'done', 'timenow', 'time_taken'])) {
                continue;
            }
    ...
    public function import_proxmox($post)
    {
    ...
    public function import_feathur($post)
    {
    ...
    public function import_hypervm($post)
    {
    ...
    public function import_openvz($post)
    {
        $post['sa'] = 'openvz';
        $res = $this->import($post);
    ...
    public function import_kvm($post)
    {
        $post['sa'] = 'kvm';
        $res = $this->import($post);
    ...
    public function import_xen($post)
    {
        $post['sa'] = 'xen';
        $res = $this->import($post);
    ...
    public function import_xcp($post)
    {
        $post['sa'] = 'xcp';
        $res = $this->import($post);
    ...
    public function import_openvz7($post)
    {
        $post['sa'] = 'openvz7';
        $res = $this->import($post);
     ...
     */
    public function ips($page = 1, $reslen = 50, $post = [])
    {
        if (empty($post)) {
    ...
    public function update_vps_net_rules($vpsid)
    {
        if (! empty($vpsid)) {
    ...
    public function performance($serid, $option = '')
    {
    ...
    {
        $post['reos'] = 1;
        $vps = $this->listvs(1, 1, ['vpsid' => $post['vpsid']]);
     ...
     * @return array The unserialized array on success OR false on failure
     */
    public function serverinfo($serid = 0)
    {
     ...
     * @return array The unserialized array on success OR false on failure
     */
    public function servers($search = [], $del_serid = 0)
    {
        if ($del_serid == 0) {
     ...
     */
    public function suspend($vid, $post = [])
    {
    ...
    public function lock($vid, $reason = '')
    {
        $path = 'index.php?act=vs&action=lock&vpsid='.$vid;
        $res = $this->call($path, [], ['reason' => $reason]);

        return $res;
    }

    public function unlock($vid)
    {
    ...
    public function resetbandwidth($vpsid)
    {
    ...
    public function list_sshkeys($uid)
    {
        if (empty($uid)) {
            return false;
        }
        $post['uid'] = $uid;
        $res = $this->call('index.php?act=users&list_ssh_keys=1', [], $post);
    ...
    public function add_sshkeys($post)
    {
        if (empty($post['vpsid'])) {
            return false;
        }
        $res = $this->call('index.php?act=managevps&vpsid='.(int) $post['vpsid'].'&add_ssh_keys=1', [], $post);
    ...
    public function list_volumes($search = [], $page = 1, $reslen = 50)
    {
    ...
    public function add_volumes($post)
    {
    ...
    public function perform_volumes_actions($post)
    {

        if (empty($post['disk_did_action'])) {
            return false;
        }
        $path = 'index.php?act=volumes';
        $res = $this->call($path, [], $post);
    ...
    public function get_load_balancer($page = 1, $reslen = 50, $post = [])
    {
    ...
    public function delete_load_balancer($lb_uuid, $delete_added_vm = 0)
    {
    ...
    public function add_to_load_balancer($lb_uuid, $vpsid)
    {
        $path = 'index.php?act=load_balancer';
    ...
    public function manage_load_balancer($lb_uuid, $post = [])
    {
    ...
    public function passthrough_pool($search = [], $page = 1, $reslen = 50)
    {
        if (empty($search)) {
    ...
    public function addpassthrough($post)
    {
        ...
        $res = $this->call('index.php?act=addpassthrough', [], $post);
        $result['done'] = $res['done'];
        if (! empty($res['error'])) {
            $result['error'] = $res['error'];
        }
    ...
    public function editpassthrough($post)
    {
        ...
        $res = $this->call('index.php?act=editpassthrough&pid='.$post['pid'], [], $post);
    ...
    public function delpassthrough($post)
    {
        ...
        $res = $this->call('index.php?act=passthrough&delete='.$post['delete'], [], $post);
